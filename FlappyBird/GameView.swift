        ...
        GeometryReader { screenSettings in
            NavigationStack {
                ZStack {
                    ...
                    .offset(x: screenSettings.size.width + pipeOffset)
                ...
                .onReceive(timer) { currentTime in
                    ...
                    checkBoundaries(geometry: screenSettings)
                    updatePipePosition(deltaTime: deltaTime)
                    resetPipePositionIfNeeded(geometry: screenSettings)
                    
                    if checkCollision(with: screenSettings) {
                        gameState = .stopped
                    }
                    
                    updateScoresAndHighScore(geometry: screenSettings)
